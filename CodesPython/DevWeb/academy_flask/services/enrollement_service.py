...
class InscriptionService:
    ...
    def __init__(self,
                 fichier_cours="./data/courses.json",
                 fichier_users="./data/users.json"
                 ):
    ...
    def recharger_cours(self):
        ...
        self.cours = [course.from_dict(d) for d in raw_cours]
    ...
    def get_etudiant(self, etu_id):
        for u in self.utilisateurs:
            if u["id"] == etu_id and u["role"].lower() == "student":
                return u
        return None
    ...
    # Renvoie une liste de dict : {user_id, nom, course_id, titre_cours}
    # -----------------------------------------------
    def lister_inscriptions(self):
        ...
        result = []
        for c in self.cours:
            for etu_id in c.etudiants_inscrits:
                etudiant = self.get_etudiant(etu_id)
                if etudiant:
                    result.append({
                        "user_id": etu_id,
                        "nom": etudiant["nom"],
                        "course_id": c.code_cours,
                        "titre_cours": c.titre
                    })
        return result
