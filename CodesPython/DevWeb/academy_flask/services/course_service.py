from utils.json_manager import load_json, save_json
from Models.course import course


class Cours_service:
    def __init__(self, fichier="./data/courses.json"):
        self.fichier = fichier

        # Charger la liste des cours sous forme de dictionnaires
        data = load_json(self.fichier)

        # Convertir chaque dictionnaire en objet Cours
        self.cours = [course.from_dict(d) for d in data]

    def ajouter_cours(self, code_cours, titre, enseignant, administrateur, capacite_cours):
        cours = course(
            code_cours,
            titre,
            enseignant,
            administrateur,
            capacite_cours
        )
        # Ajouter l'objet dans la liste
        self.cours.append(cours)

        # Sauvegarder la liste mise à jour
        self._sauvegarder()
        return cours

    def supprimer_cours(self, code_cours):
        for c in self.cours:
            if c.code_cours == code_cours:
                self.cours.remove(c)
                self._sauvegarder()
                return True
        return False

    def lister_cours(self):
        return self.cours

    def chercher_cours(self, code_cours):
        for c in self.cours:
            if c.code_cours == code_cours:
                return c
        return None

    def _sauvegarder(self):
        """Sauvegarde interne : convertit les objets en dictionnaires."""
        data = [c.to_dict() for c in self.cours]
        save_json(self.fichier, data)

    
    def modifier_cours(self, code_cours, titre=None, enseignant=None, administrateur=None, capacite_cours=None):
        cours = self.chercher_cours(code_cours)
        if not cours:
            return None
        if titre:
            cours.titre = titre
        if enseignant:
            cours.enseignant_id = enseignant
        if administrateur:
            cours.administrateur_id = administrateur
        if capacite_cours is not None:
            cours.capacite_cours = capacite_cours
        self._sauvegarder()
        return cours
    
    def get_course_by_id(self, code_cours):
    #Retourne l'objet course correspondant au code_cours,
    #ou None si introuvable.
        for c in self.cours:
            if c.code_cours == code_cours:
                return c
        return None
