...
def sauvegarde_JSON():
    with open(fichier_JSON, 'w', encoding='utf-8') as file:
        #json.dump(etudiant, file) # il les mets  
...
def affich_etudiants():
    if etudiants_json:  # le dictionnaire n’est pas vide
        print("\n--- Liste des étudiants ---")
        for matricule, infos in etudiants_json.items():
